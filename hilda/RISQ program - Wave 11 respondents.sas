...
* Create runtimes file ;
	filename runtimes "&logsloc.\Run Times.txt";
	%put ;
...
%macro risqprogram(wave,sample,period) ;

proc printto new
	log = "&logsloc.\RISQLog_w11r_wave&wave._&period._&sample..log"
	print = "&logsloc.\RISQResults_w11r_wave&wave._&period._&sample..lst" ;
run ;

* Indicate start wave, sample, period combination ;
%logentry( fref = runtimes , msg = "Starting wave &wave., &sample. sample, period &period. ") ;
...
data  att;
set risq.propensities_wave11respondents; /* Updated from controlcbs to the name of the downloaded dataset */
...
%if &period=P2 %then %do ;
	if tracking_day117=1 then responsesamp1=1;
	else if tracking_day117 in (0,2) then responsesamp1=0;
%end ;
%if &period=P3 %then %do ;
	if tracking_day149=1 then responsesamp1=1;
	else if tracking_day149 in (0,2) then responsesamp1=0;
%end ;
...
/***** final output file - names and directory of output can be changed here **********/
 ...
 PROC EXPORT DATA= RISQ.FINAL_OUTPUT_EX2 
            OUTFILE= "&workloc.\RISQoutput_w11r_wave&wave._&period._&sample." 
            DBMS=XLSX REPLACE;
     PUTNAMES=YES;
RUN;
	...
	%do wave = &startwave %to &endwave ;
		%do s = 1 %to 3 ;
			%do p = 1 %to 3 ;
				...
				%risqprogram(&wave,&sample,&period)
			%end ;
		%end ;
	%end ;
