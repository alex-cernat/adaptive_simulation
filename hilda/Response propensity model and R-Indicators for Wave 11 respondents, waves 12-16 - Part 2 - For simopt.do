...
gen response_P1_opt`opt' = 1 if tracking_day69==1
replace response_P1_opt`opt' = 0 if inlist(tracking_day69,0,2)

forvalues period = 1/3 {
	/* Calculate overall R indicators and coefficients of variation */
	xi:logistic response_P`period'_opt`opt' $predictors_subsample
	lroc
...
args period wave opt

	clear
	import excel "$workinglocation\RISQoutput_w11r_simopt`opt'_wave`wave'_`period'_Overall.xlsx" , firstrow case(preserve)
...
args period sample wave opt

	clear
	import excel "$workinglocation\RISQoutput_w11r_simopt`opt'_wave`wave'_`period'_`sample'.xlsx" , firstrow case(preserve)
...
forvalues period = 1/3 {
	...
	/* Calculate overall R indicators and coefficients of variation */
	xi:probit response_P`period'_opt`opt' $predictors
	predict presponse_P`period'_allvars , pr
	sum presponse_P`period'_allvars 
	gen rind_P`period'_allvars = 1-2*r(sd)
	gen rsd_P`period' = r(sd)
...
subsamples 1 `wave' `opt'
subsamples 2 `wave' `opt'

* Now combine these results with the results from the SAS RISQ program:
* For subsamples:

risqresults_subsample "P1" "Main" `wave' `opt'
risqresults_subsample "P1" "TopUp" `wave' `opt'
risqresults_subsample "P2" "Main" `wave' `opt'
risqresults_subsample "P2" "TopUp" `wave' `opt'
risqresults_subsample "P3" "Main" `wave' `opt'
risqresults_subsample "P3" "TopUp" `wave' `opt'

groupperiodresults "Main" `wave' `opt'
groupperiodresults "TopUp" `wave' `opt'

use "$workinglocation\RIndicatorsAndCVs_Main", clear
export  excel "$resultslocation\Phase 2 - R-Indicators and CVs - Wave 11 respondents and characteristics - simopt`opt'.xlsx" , ///
...
list
export  excel "$resultslocation\Phase 3 - Results grouped across waves - Wave 11 respondents and characteristics - simopt`opt'.xlsx" , ///
...
forvalues wave=$startwave/$endwave {
	forvalues period=1/3 {
		clear
		import excel "$workinglocation\RISQoutput_w11r_simopt`opt'_wave`wave'_P`period'_`sample'.xlsx" , firstrow case(preserve)
		keep r_indicator 
		drop if r_indicator==.
		gen Wave=`wave'
...
export  excel "$resultslocation\Phase 4 - RIndicators across waves - Wave 11 respondents and characteristics - simopt`opt'.xlsx" , ///
...
forvalues opt=1/9 {
	...
	forvalues wave=$startwave/$endwave {
		rindicators `wave' `opt'
	}
	
	groupwavegraphs 1 `opt'
	groupwavegraphs 3 `opt'
	...
	rindgraphs Main `opt'
	rindgraphs TopUp `opt'
	
	rindgraphsse Main `opt'
	rindgraphsse TopUp `opt'
	
	cvgraphs Main `opt'
	cvgraphs TopUp `opt'
	
	cvgraphsse Main `opt'
	cvgraphsse TopUp `opt'
}
