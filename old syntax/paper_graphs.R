...
ggsave("./results/rr_plot.png", dpi = 500, height = 7)
...
ggsave("./results/rr_plot_balanced.png", dpi = 500, height = 7)
...
rinds <- rbind(hilda_rinds, us_rinds) %>% 
  mutate(level = as.factor(level) %>% fct_rev())


rinds %>% 
  ggplot(aes(as.factor(wave), est, 
             color = sim, 
             group = sim)) +
  geom_point() + geom_line() +
  geom_errorbar(aes(ymin = lci, ymax = uci), 
                width = 0, alpha = 0.5) +
  facet_wrap(sample ~ level, scales = "free_x", ncol = 2) +
  theme_bw() +
...
rinds %>% 
  ...
  geom_errorbar(aes(ymin = lci, ymax = uci), 
                width = 0, alpha = 0.5) +
  facet_wrap(sample ~ level, scales = "free", ncol = 2) +
  theme_bw() +
  labs(y = "R indicator", x = "Wave", color = "Simulation",
       caption = "Please note change of scales")
